...
| 항목 | 구현값 | 조문 | 확인 |
|---|---|---|---|
| 하중조합 U1~U8 | [하중조합 문서](kds_loads.md#하중조합) 의 표 | KDS 14 20 10 4.2.2 | ☐ |
| 풍하중 계수 | 1.3 (U5, U7), 0.65 (U4) | KDS 14 20 10 4.2.2 | ☐ |
| 활하중 저감 | 1.0 → 0.5 | KDS 14 20 10 4.2.2 | ☐ |

### 전단 및 비틀림 (KDS 14 20 22)

| 항목 | 구현값 | 조문 | 확인 |
|---|---|---|---|
| 강도감소계수 | 0.75 | KDS 14 20 10 4.3.3(2) | ☐ |
...
| 균열 제어 철근 간격 | $375(\kappa_{cr}/f_s) - 2.5c_c \le 300(\kappa_{cr}/f_s)$ | KDS 14 20 20 4.2.3(4) | ☐ |
| $\kappa_{cr}$ | 280 (건조) / 210 (그 밖) | KDS 14 20 20 4.2.3(4) | ☐ |
| 수축·온도철근비 | 0.0020 / $\max(0.0020\cdot400/f_y,\ 0.0014)$ | KDS 14 20 50 4.6.2 | ☐ |
...
| 항목 | 구현값 | 조문 | 확인 |
|---|---|---|---|
| 긴장재 허용응력 | $\min(0.80f_{pu}, 0.94f_{py})$ / $0.70f_{pu}$ | KDS 14 20 60 4.2 | ☐ |
| 콘크리트 도입 직후 | 압축 $0.60f_{ci}$, 인장 $0.25\sqrt{f_{ci}}$ | KDS 14 20 60 4.2 | ☐ |
| 콘크리트 사용하중 | 압축 $0.45f_{ck}$ / $0.60f_{ck}$ | KDS 14 20 60 4.2 | ☐ |
| 균열등급 인장 한계 | U $0.63\sqrt{f_{ck}}$, T $1.0\sqrt{f_{ck}}$ | KDS 14 20 60 4.2 | ☐ |
| 마찰 손실 | $P_{pj}e^{-(\mu\alpha + Kl)}$ | KDS 14 20 60 4.3 | ☐ |
| 릴랙세이션 | $k$ = 45 (저릴랙세이션) / 10 | KDS 14 20 60 4.3 | ☐ |
| 부착 $f_{ps}$ | $\gamma_p$ = 0.55 / 0.40 / 0.28 | KDS 14 20 60 4.1 | ☐ |
| 비부착 $f_{ps}$ | $f_{pe}+70+f_{ck}/(100\rho_p)$ 등 | KDS 14 20 60 4.1 | ☐ |
