...
아래 판의 **원문(hwp)과 직접 대조**하여 계수와 조문 번호를 확인하였다.
KDS 는 계속 개정되므로, 이보다 새 판이 고시되었다면 다시 대조해야 한다.

| 코드 | 제목 | 판 | 고시일 | 사용 모듈 |
|---|---|---|---|---|
| KDS 14 20 10 | 콘크리트구조 해석과 설계 원칙 | **2021** | 2021-02-18 개정 | `kds`, `loads` |
| KDS 14 20 20 | 콘크리트구조 휨 및 압축 설계기준 | **2022** | 2022-01-11 개정 | `kds`, `slender` |
| KDS 14 20 22 | 콘크리트구조 전단 및 비틀림 설계기준 | **2022** | 2022-01-11 개정 | `shear` |
| KDS 14 20 30 | 콘크리트구조 사용성 설계기준 | **2021** | 2021-02-18 개정 | `serviceability` |
| KDS 14 20 40 | 콘크리트구조 내구성 설계기준 | **2022** | 2022-01-10 개정 | `durability` |
| KDS 14 20 50 | 콘크리트구조 철근상세 설계기준 | **2022** | 2022-01-11 개정 | `detailing` |
| KDS 14 20 52 | 콘크리트구조 정착 및 이음 설계기준 | **2024** | 2024-12-30 개정 | `detailing` |
| KDS 14 20 60 | 프리스트레스트 콘크리트구조 설계기준 | **2022** | 2022-01-11 개정 | `psc` |

상위 기준인 **KDS 14 20 01** 콘크리트구조 설계(강도설계법) 일반사항은 2022 판
(2022-01-11 개정)이며, 용어와 설계 원칙의 근거로만 참조한다. 하중계수와
하중조합은 이 기준이 아니라 **KDS 14 20 10 4.2.2** 에 있다.

범위 밖으로 둔 기준의 판은 다음과 같다 — KDS 14 20 24 스트럿-타이(2021),
KDS 14 20 70 슬래브와 기초판(2021), KDS 14 20 72 벽체(2021),
KDS 14 20 80 내진설계(2021). 자세한 사유는
[설계기준 문서](docs/user_guide/design_codes/kds.md#다루지-않는-범위)에 적었다.
...
│   ├── loads.py           하중조합        KDS 14 20 10 4.2.2
...
> 계수와 조문 번호는 위 [적용 기준 판](#적용-기준-판)의 원문과 대조하였고,
> 항목별 결과는
> [docs/user_guide/design_codes/kds.md](docs/user_guide/design_codes/kds.md#기준-값-출처와-검증)
> 의 **검증 대조표**(항목 70여 개)에 정리해 두었다. 대조 과정에서 계산에
> 영향을 주는 오류 6건과 조문 인용 오류 7건을 고쳤으며, 그 목록도 같은 문서에
> 있다.
>
> 다만 KDS 는 계속 개정된다. 위 표의 판보다 새 고시가 있었다면 적용 전에 다시
> 대조해야 한다. 특히 개정이 잦은 곳은 다음과 같다.
>
> - KDS 14 20 20 표 4.1-2 의 $\eta$, $\beta_1$, $\varepsilon_{cu}$ ($f_{ck}$ 50~90 MPa)
> - KDS 14 20 10 4.2.2 의 하중계수와 하중조합
> - KDS 14 20 22 의 전단 규정
> - KDS 14 20 40 의 노출등급별 요구값
> - KDS 14 20 52 는 2024-12 개정으로 정착길이 규정이 바뀌었다
>
> 결과의 확인과 채택은 사용자의 책임이다.
