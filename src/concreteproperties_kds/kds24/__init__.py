"""KDS 24 (교량설계기준, 한계상태설계법) 모듈.

KDS 14 가 강도설계법이라면 KDS 24 는 한계상태설계법이다. 두 기준은 같은
역학을 쓰지만 **안전율을 거는 자리가 다르다** — KDS 14 는 공칭강도에
강도감소계수를 한 번 곱하고, KDS 24 는 재료마다 재료계수를 곱해 설계
재료강도를 만든다.

구현 범위

- ``materials`` — 재료계수(표 1.4-1), 설계 재료강도, 단면설계용 응력-변형률
  (KDS 24 14 21 1.4, 3.1)
- ``design_code`` — :class:`KDS24` 설계기준 클래스, 극한한계상태 휨·축력
  (KDS 24 14 21 4.1.1)
- ``loads`` — 하중조합과 하중수정계수 (KDS 24 12 11 4.1, KDS 24 10 11 1.3)
- ``live_load`` — 차량활하중 KL-510, 충격, 다차로 재하 (KDS 24 12 21 4.3, 4.4)
- ``shear`` — 변각 트러스 모델 전단 (KDS 24 14 21 4.1.2)
- ``serviceability`` — 응력 한계·균열폭·처짐·피로 (KDS 24 14 21 4.2, 4.3)
- ``deck`` — 교량 바닥판 설계 (KDS 24 10 11 4.6.2, KDS 24 14 21 4.6.5)
- ``psc`` — 프리스트레스 도입응력과 손실 (KDS 24 14 21 1.5.7, 3.3)
- ``girder`` — PSC I형 거더 단면과 설계 (KDS 24 14 21 4.1, 4.2)

두 기준의 대응은 문서의 [KDS 14 와 KDS 24 비교] 를 참고한다.
"""

from __future__ import annotations

from . import deck, girder, live_load, loads, psc, serviceability, shear
from .deck import (
    BAR_SPACING_MAX,
    BAR_SPACING_MIN,
    COVER_DURABILITY,
    DECK_FCK,
    MIN_THICKNESS_EMPIRICAL,
    MIN_THICKNESS_PSC,
    MIN_THICKNESS_RC,
    WHEEL_LOAD,
    DeckDesign,
    bar_area,
    cantilever_live_load_moment,
    cantilever_wheel_width,
    dead_load_moment,
    deck_deflection_limit,
    deck_span,
    design_deck,
    distribution_steel_ratio,
    live_load_moment,
    live_load_moment_parallel,
    minimum_flexural_steel,
    nominal_cover,
    provided_steel_area,
    required_steel_area,
    wheel_width_parallel,
)
from .design_code import (
    KDS24,
    biaxial_exponent,
    minimum_eccentricity,
)
from .girder import (
    EX_BOTTOM_FLANGE_TIP,
    EX_BOTTOM_TAPER,
    EX_MAX_DEPTH,
    EX_MAX_SPAN,
    EX_WEB_THICKNESS,
    EXAMPLE_SECTIONS,
    GAMMA_CONCRETE,
    TENDON_COVER,
    GirderCheck,
    IGirder,
    SectionProperties,
    design_girder,
)
from .live_load import (
    FATIGUE_TRUCK_RATIO,
    IMPACT_PERCENT,
    IMPACT_PERCENT_FATIGUE,
    LANE_LOAD_BASE,
    TRUCK_AXLE_LOADS,
    TRUCK_AXLE_POSITIONS,
    TRUCK_TOTAL_LOAD,
    LiveLoadEffect,
    adtt_single_lane,
    fatigue_truck_moment,
    girder_live_load,
    impact_buried,
    impact_factor,
    lane_load,
    lane_moment,
    lane_shear,
    lane_width,
    multiple_presence,
    number_of_lanes,
    truck_lane_fraction,
    truck_moment,
    truck_shear,
)
from .loads import (
    BRIDGE_GRADE_FACTORS,
    COMBINATIONS_BY_NAME,
    LOAD_COMBINATIONS,
    LOAD_SYMBOLS,
    PERMANENT_LOAD_FACTORS,
    LoadCombination,
    bridge_grade_factor,
    evaluate_all,
    governing_combination,
    load_modifier,
    permanent_load_factor,
)
from .materials import (
    ALPHA_CC,
    ES,
    MATERIAL_FACTORS,
    PHI_C_ULS,
    PHI_S_ULS,
    characteristic_tensile_strength,
    concrete_curve_table,
    curve_parameters,
    design_compressive_strength,
    design_profile,
    design_stress,
    design_tensile_strength,
    design_yield_strength,
    elastic_modulus,
    equivalent_block,
    material_factors,
    mean_compressive_strength,
    mean_tensile_strength,
)
from .psc import (
    CONCRETE_CREEP_LINEAR_RATIO,
    CONCRETE_TRANSFER_RATIO,
    CONCRETE_TRANSFER_RATIO_PRE,
    CURVATURE_FRICTION,
    E_P,
    FINAL_RELAXATION_HOURS,
    JACKING_RATIO_FPU,
    JACKING_RATIO_FPY,
    OVERTENSION_RATIO_FPY,
    RELAXATION_COEFFICIENTS,
    RHO_1000,
    TRANSFER_RATIO_HIGH,
    TRANSFER_RATIO_LOW,
    WOBBLE_RANGE,
    PrestressLosses,
    anchorage_set_loss,
    concrete_stress_limit_at_transfer,
    elastic_shortening_loss,
    friction_loss,
    long_term_loss,
    max_jacking_stress,
    relaxation_loss,
    stress_after_transfer,
)
from .serviceability import (
    COUPLER_FATIGUE_STRENGTH,
    DESIGN_GRADES,
    EXPOSURE_MINIMUM_GRADE,
    MAX_BAR_DIAMETER,
    MAX_BAR_SPACING,
    CrackWidthCheck,
    DesignGrade,
    concrete_stress_limit,
    coupler_fatigue_strength,
    crack_spacing,
    crack_spacing_unreinforced,
    crack_width,
    deflection_limit,
    effective_tension_depth,
    fatigue_check_required,
    fatigue_stress_range_limit,
    max_bar_diameter,
    max_bar_spacing,
    minimum_crack_reinforcement,
    minimum_design_grade,
    nonuniform_stress_factor,
    steel_stress_limit,
    strain_difference,
    stress_distribution_factor,
    tendon_stress_limit,
    web_effective_tensile_strength,
)
from .shear import (
    COT_THETA_MAX,
    COT_THETA_MIN,
    ShearCheck,
    alpha_cw,
    axial_stress,
    check_shear,
    concrete_shear_strength,
    design_concrete_shear_strength,
    kappa,
    max_shear_strength,
    maximum_shear_reinforcement,
    maximum_stirrup_spacing,
    minimum_concrete_shear_strength,
    minimum_shear_reinforcement_ratio,
    nu,
    required_stirrup_spacing,
    shear_reinforcement_strength,
    uncracked_shear_strength,
)

__all__ = [
    "ALPHA_CC",
    "BAR_SPACING_MAX",
    "BAR_SPACING_MIN",
    "BRIDGE_GRADE_FACTORS",
    "COMBINATIONS_BY_NAME",
    "CONCRETE_CREEP_LINEAR_RATIO",
    "CONCRETE_TRANSFER_RATIO",
    "CONCRETE_TRANSFER_RATIO_PRE",
    "COT_THETA_MAX",
    "COT_THETA_MIN",
    "COUPLER_FATIGUE_STRENGTH",
    "COVER_DURABILITY",
    "CURVATURE_FRICTION",
    "CrackWidthCheck",
    "DECK_FCK",
    "DESIGN_GRADES",
    "DeckDesign",
    "DesignGrade",
    "ES",
    "EXAMPLE_SECTIONS",
    "EXPOSURE_MINIMUM_GRADE",
    "EX_BOTTOM_FLANGE_TIP",
    "EX_BOTTOM_TAPER",
    "EX_MAX_DEPTH",
    "EX_MAX_SPAN",
    "EX_WEB_THICKNESS",
    "E_P",
    "FATIGUE_TRUCK_RATIO",
    "FINAL_RELAXATION_HOURS",
    "GAMMA_CONCRETE",
    "GirderCheck",
    "IGirder",
    "IMPACT_PERCENT",
    "IMPACT_PERCENT_FATIGUE",
    "JACKING_RATIO_FPU",
    "JACKING_RATIO_FPY",
    "KDS24",
    "LANE_LOAD_BASE",
    "LOAD_COMBINATIONS",
    "LOAD_SYMBOLS",
    "LiveLoadEffect",
    "LoadCombination",
    "MATERIAL_FACTORS",
    "MAX_BAR_DIAMETER",
    "MAX_BAR_SPACING",
    "MIN_THICKNESS_EMPIRICAL",
    "MIN_THICKNESS_PSC",
    "MIN_THICKNESS_RC",
    "OVERTENSION_RATIO_FPY",
    "PERMANENT_LOAD_FACTORS",
    "PHI_C_ULS",
    "PHI_S_ULS",
    "PrestressLosses",
    "RELAXATION_COEFFICIENTS",
    "RHO_1000",
    "SectionProperties",
    "ShearCheck",
    "TENDON_COVER",
    "TRANSFER_RATIO_HIGH",
    "TRANSFER_RATIO_LOW",
    "TRUCK_AXLE_LOADS",
    "TRUCK_AXLE_POSITIONS",
    "TRUCK_TOTAL_LOAD",
    "WHEEL_LOAD",
    "WOBBLE_RANGE",
    "adtt_single_lane",
    "alpha_cw",
    "anchorage_set_loss",
    "annotations",
    "axial_stress",
    "bar_area",
    "biaxial_exponent",
    "bridge_grade_factor",
    "cantilever_live_load_moment",
    "cantilever_wheel_width",
    "characteristic_tensile_strength",
    "check_shear",
    "concrete_curve_table",
    "concrete_shear_strength",
    "concrete_stress_limit",
    "concrete_stress_limit_at_transfer",
    "coupler_fatigue_strength",
    "crack_spacing",
    "crack_spacing_unreinforced",
    "crack_width",
    "curve_parameters",
    "dead_load_moment",
    "deck",
    "deck_deflection_limit",
    "deck_span",
    "deflection_limit",
    "design_compressive_strength",
    "design_concrete_shear_strength",
    "design_deck",
    "design_girder",
    "design_profile",
    "design_stress",
    "design_tensile_strength",
    "design_yield_strength",
    "distribution_steel_ratio",
    "effective_tension_depth",
    "elastic_modulus",
    "elastic_shortening_loss",
    "equivalent_block",
    "evaluate_all",
    "fatigue_check_required",
    "fatigue_stress_range_limit",
    "fatigue_truck_moment",
    "friction_loss",
    "girder",
    "girder_live_load",
    "governing_combination",
    "impact_buried",
    "impact_factor",
    "kappa",
    "lane_load",
    "lane_moment",
    "lane_shear",
    "lane_width",
    "live_load",
    "live_load_moment",
    "live_load_moment_parallel",
    "load_modifier",
    "loads",
    "long_term_loss",
    "material_factors",
    "max_bar_diameter",
    "max_bar_spacing",
    "max_jacking_stress",
    "max_shear_strength",
    "maximum_shear_reinforcement",
    "maximum_stirrup_spacing",
    "mean_compressive_strength",
    "mean_tensile_strength",
    "minimum_concrete_shear_strength",
    "minimum_crack_reinforcement",
    "minimum_design_grade",
    "minimum_eccentricity",
    "minimum_flexural_steel",
    "minimum_shear_reinforcement_ratio",
    "multiple_presence",
    "nominal_cover",
    "nonuniform_stress_factor",
    "nu",
    "number_of_lanes",
    "permanent_load_factor",
    "provided_steel_area",
    "psc",
    "relaxation_loss",
    "required_steel_area",
    "required_stirrup_spacing",
    "serviceability",
    "shear",
    "shear_reinforcement_strength",
    "steel_stress_limit",
    "strain_difference",
    "stress_after_transfer",
    "stress_distribution_factor",
    "tendon_stress_limit",
    "truck_lane_fraction",
    "truck_moment",
    "truck_shear",
    "uncracked_shear_strength",
    "web_effective_tensile_strength",
    "wheel_width_parallel",
]
