...
바닥판은 다리에서 **가장 먼저 망가지는 부재**다. 거더는 50년을 가도 바닥판은
...
def live_load_moment(
    ...
    r"""주철근이 차량진행방향에 **직각**인 바닥판의 활하중 휨모멘트.
    ...
    **KDS 24 10 11 4.6.2.4(2)① 식 (4.6-1)**
    ...
    지점이 셋 이상인 연속슬래브의 정모멘트는 이 값의 **0.8 배**를 쓴다.
...
def wheel_width_parallel(span: float) -> float:
    r"""주철근이 차량진행방향에 **평행**할 때 윤하중이 퍼지는 폭 :math:`E`.
...
def live_load_moment_parallel(span: float, grade: int = 1) -> float:
    r"""주철근이 차량진행방향에 **평행**한 단순판의 활하중 휨모멘트.
